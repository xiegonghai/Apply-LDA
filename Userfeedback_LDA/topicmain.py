...
db = MySQLdb.connect(host=setting.DATABASE['host'],user=setting.DATABASE['user'],passwd=setting.DATABASE['passwd'],db=setting.DATABASE['db'],port=setting.DATABASE['port'],charset='utf8')
...
timestamp = nowtime + datetime.timedelta(days=-setting.day_num)
...
def read_news():
    ...
    sql = "SELECT * FROM %s WHERE substr(time,1,10)>%s and (productID =101 or productID =0)" % (table_name,keepdate)
    cursor.execute(sql);
...
def preprocessing():
    ...
    for row in news_list:
        line = re.findall(ur"[\u4e00-\u9fa5]",row,re.MULTILINE)
        line = ''.join(line)
        chinese_list.append(line)
##################################################################################################################################
def load_stopwords():
